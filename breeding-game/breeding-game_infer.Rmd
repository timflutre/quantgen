...
```{r}
u <- cbind(p0$trait1$u, p0$trait2$u)
mean(u[,1])
mean(u[,2])
var(u[,1])
var(u[,2])
cor(u[,1], u[,2])
regplot(u[,1], u[,2], main="True values ('biological', not 'statistical')",
        xlab="Genotypic values of trait1", ylab="Genotypic values of trait2")
```
...
Remove known confounders for both traits simultaneously (a bit slow):
```{r, eval=FALSE}
system.time(
