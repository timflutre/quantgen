...
```{r}
f <- paste0(setup$init.dir, "/genos_coll-latest-year.txt.gz")
tools::md5sum(path.expand(f))
...
if(! file.exists(Sys.which("gemma"))){
  ...
  res3 <- list()
  res3$vc <- estVC(y=blups3[rownames(A.mark)],
                   v=list(AA=A.mark, DD=NULL, HH=NULL,
                          AD=NULL, MH=NULL, EE=diag(length(blups3))))
  idx.snps <- 1:ncol(genos)
  res3$scan <- scanOne(y=blups3[rownames(genos)], gdat=genos[,idx.snps],
