...
Note that, when $\boldsymbol{g}$ is simulated as $X \boldsymbol{\beta}$ with $X$ in $\{0,1,2\}$, its mean isn't $\boldsymbol{0}$k.
For this to be the case, the $X$ matrix should be centered using the allele frequencies from the base population.
...
In the course of the game, it would be better to update the allele frequencies.
...
We deduce the (additive) genotypic variance given that the genotypic coef of variation is set at a particular value: $\sigma_a = \text{CV}_g \times \mu$.
...
## h2.traits12 <- c(trait1=params.trait1$h2, trait2=params.trait2$h2)
...
BVs <- data.frame(trait1=p0$mu[1] + p0$G.A[,1],
                  ...
                  trait3=! coll$genos[levels(p0.df$ind), p0$trait3$qtn.id] %in% p0$trait3$resist.genos,
...
## Choose registration thresholds

```{r}
register.min.trait1 <- 1.03 # compare to the median of the controls
register.min.trait2 <- mu.traits12[2]
```

TODO

As designer of the game, we can set the difficulty of the requirements by tuning the required response to selection, and thus the number of standard deviations necessary to be registered.

$R = H^2_{\text{design}} \; i \; \sigma_p^\star$
...
Save also the initial allele frequencies into a file per breeder:
```{r save_afs_in_file_per_breder}
for(breeder in breeders){
  f <- paste0(truth.dir, "/", breeder, "/afs.RData")
  afs.breeder <- afs[thin.snps]
  save(afs.breeder, file=f)
}
```
