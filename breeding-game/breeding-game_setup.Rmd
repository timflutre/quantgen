...
if(! file.exists(f)){
  ...
  coll$genos <- segSites2allDoses(coll$haplos, crosses$child,
                                  colnames(g0$genos))
...
p0.df <- data.frame(ind=NA,
                    year=rep(2005:(2005+J-1), each=L),
                    plot=rep(1:L, times=J),
                    pathogen=NA,
                    trait1.raw=NA,
                    trait1=NA,
                    trait2=NA,
                    trait3=NA)
p0.df$year <- as.factor(p0.df$year)
p0.df$plot <- as.factor(p0.df$plot)
...
p0.df$pathogen[p0.df$year %in% c(2005, 2008, 2011, 2014)] <- TRUE
```

Export genotypes for the players using high-density chip, but only for lines planted the last year (2014):
```{r export_geno}
f <- paste0(init.dir, "/genos_coll-2014.txt.gz")
subset.coll <- unique(as.character(p0.df$ind[p0.df$year == 2014]))
...
traits12 <- simulTraits12Rnd(h2=c(0.3, 0.4), sigma.beta2=c(10^(-5), 10^(-5)),
                             ...
                             X=p0$X, center.genos=TRUE, plot=TRUE)
```
...
These are chosen so that they have (i) a "genotypic yield" among the 80% best, (ii) a "genotypic quality" above 14/1000, and (iii) the resistant allele.

Rank the genotypes:
```{r}
genovals <- data.frame(trait1=p0$trait1$mu + (p0$X - 1) %*% p0$trait1$beta,
                       trait2=p0$trait2$mu + (p0$X - 1) %*% p0$trait2$beta,
                       trait3=! rownames(p0$X) %in% p0$trait3$inds.resist)
...
```{r}
nrow(potential.controls <- genovals[genovals$trait2 >= 14 & ! genovals$trait3,])
...
```{r}
(controls <- rownames(potential.controls)[(nrow(potential.controls)-4):nrow(potential.controls)])
...
tmp.ids <- gsub("Coll", "Ind", ind.ids)
for(ind.id in ind.ids){
