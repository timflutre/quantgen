...
function parseCmdLine () {
	...
	TEMP=`getopt -o hVv:I:i:p:o:c -l help,version,verbose:,zip:,png:,prefix:,out:,clean, \
        -n "$0" -- "$@"`
    else # original getopt is available (no long options, whitespace, sorting)
	TEMP=`getopt hVv:I:i:p:o:c "$@"`
    fi
    if [ $? -ne 0 ]; then
...
function run () {
    ...
    if [ $verbose -gt "0" ]; then
        echo "concatenate $(ls ${globPng} | wc -l) PNG files into a PDF file ..."
    fi
    rm -f ${outPdf}
...
run globZip globPng fastqcPrefix outPdf clean verbose
