...
barplot.nreads.fastqc <- function(x, main=""){
    ...
    text(bp, par("usr")[3], srt=45, adj=1.1,cex=0.5, labels=names(sort(x)), xpd=TRUE)
}
...
##' @param Freq=T to plot the distribution (ylab="Phred Frequency") or Freq=T to plot read counts (ylab="Number of sequences") (default = T)
##' @param numgroup the max number of files plotted on the same graph (default = 25)
##' @return None
##' @author Timothée Flutre [cre,aut]
plot.nbseq.qual <- function(all.qc,qual,
    ...
    if(Freq==T) {
                nreads<-nreads.fastqc(all.qc)
                ylab="Phred frequency"
    }
    else {
         nreads<-rep(1,length(all.qc))  
    ...
    dividend <- nrow(qual) %/% numgroup
    if (nrow(qual)> numgroup) {
       for (k in 1:dividend) {
       plot(x=0, y=0,
         xlim=c(lowest.qual, highest.qual),
         if (Freq==F) ylim=c(lowest.count, highest.count) else ylim=c(0,1),
         xlab="Phred quality", ylab=ylab,
         main=main,
         type="n",
         bty="n")
           for (i in ((k-1)*numgroup+1):(k*numgroup)) {
               l=i-(k-1)*numgroup 
               idx <- which(! is.na(qual[i,]))
               points(x=idx, y=qual[i,idx]/nreads[i], col=l, pch=l, type="b")
           }
           axis(side=4)
           legend(x=legend.x, y=legend.y, legend=rownames(qual)[((k-1)*numgroup+1):(k*numgroup)],cex=0.7,
           col=1:numgroup, pch=1:numgroup, bty="n")
       }
    }    
    plot(x=0, y=0,
         xlim=c(lowest.qual, highest.qual),
         ...
         type="n",
         bty="n")
    for(i in (dividend*numgroup+1):nrow(qual)){
          l= i %% numgroup 
          idx <- which(! is.na(qual[i,]))
          points(x=idx, y=qual[i,idx]/nreads[i], col=l, pch=l, type="b")
    }
    axis(side=4)
    legend(x=legend.x, y=legend.y, legend=rownames(qual)[(dividend*numgroup+1):nrow(qual)],cex=0.7,
           col=1:l, pch=1:l, bty="n")
}
...
adp.contents.fastqc <- function(all.qc, adp="Illumina Universal Adapter"){
  ...
  adp.content <- matrix(NA, nrow=N, ncol=length(positions), dimnames=list(names(all.qc), positions))
  for(i in 1:N) {
        dif <-  L[max] - L[i]
        if (dif==0) { adp.content[i,] <- all.qc[[i]]$Adapter_Content[[adp]] }
        else   adp.content[i,] <- c(all.qc[[i]]$Adapter_Content[[adp]],rep(0,dif))
  }
  return(adp.content)
}
...
plot.adp.content <- function(adp.content,
    ...
    dividend <- nrow(adp.content) %/% numgroup
    if (nrow(adp.content)> numgroup) {
       for (k in 1:dividend) {
         plot(x=0, y=0,
         ...
         axis(side=4)
         legend(x=legend.x, y=legend.y, legend=rownames(adp.content)[((k-1)*numgroup+1):(k*numgroup)],
           col=1:numgroup, pch=1:numgroup, bty="n",cex=0.7)
    }
    plot(x=0, y=0,
         xlim=c(positions[1], positions[length(positions)]),
         ylim=c(lowest.adp, highest.adp),
         ...
         main=main,
         type="n")         
    for(i in (dividend*numgroup+1):nrow(adp.content)){
         l= i %% numgroup 
         points(x=positions, y=adp.content[i,], col=l, pch=l, type="b")
    }
    axis(side=4)
    legend(x=legend.x, y=legend.y, legend=rownames(adp.content)[(dividend*numgroup+1):nrow(adp.content)],
           col=1:l, pch=1:l, bty="n",cex=0.7)
    }
}
...
seq.lengths.fastqc <- function(all.qc){
  ...
  for(i in 1:N){
    ...
    if (length(grep("-",tmp$Length))!=0) { tmp$Length <- sapply(strsplit(tmp$Length, "-"), function(x){x[1]})  }
...
plot.seq.length <- function(seq.length,
  ...
  dividend <- nrow(seq.length) %/% numgroup
  if (nrow(seq.length)> numgroup) {
     for (k in 1:dividend) {
         plot(x=0, y=0,
         ...
         axis(side=4)
         legend(x=legend.x, y=legend.y, legend=rownames(seq.length)[((k-1)*numgroup+1):(k*numgroup)],
           col=1:numgroup, pch=1:numgroup, bty="n",cex=0.7)    
     }
  }
  plot(x=0, y=0,
         ...
         main=main,
         type="n")
  for(i in (dividend*numgroup+1):nrow(seq.length)){
    l= i %% numgroup 
    points(x=lengths, y=seq.length[i,], col=l, pch=l, type="b")
  }
  axis(side=4)
  legend(x=legend.x, y=legend.y, legend=rownames(seq.length)[(dividend*numgroup+1):nrow(seq.length)],
         col=1:l, pch=1:l, bty="n",cex=0.7)
}
