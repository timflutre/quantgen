...
##' @param numgroup the max number of files plotted on the same graph
##' @param main an overall title for the plot
##' @param legend.x x co-ordinate to position the legend
  ...
  if(nrow(qual) > numgroup){
    for(k in 1:dividend){
      ...
      legend(x=legend.x, y=legend.y,
             ...
             cex=legend.cex, col=1:numgroup, pch=1:numgroup, bty="n")
    }
  }
  plot(x=0, y=0,
       xlim=c(lowest.qual, highest.qual), ylim=ylim,
  ...
  if(nrow(adp.content) > numgroup){
    for(k in 1:dividend){
      ...
      legend(x=legend.x, y=legend.y,
             ...
             col=1:numgroup, pch=1:numgroup, bty="n", cex=legend.cex)
    }
  }
  plot(x=0, y=0,
       xlim=c(positions[1], positions[length(positions)]),
