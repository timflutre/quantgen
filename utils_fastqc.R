...
##' @param freq plot the distribution (ylab="Phred frequency") if TRUE, otherwise the counts (ylab="Number of sequences")
##' @param main an overall title for the plot
...
plot.nbseq.qual <- function(all.qc,qual,
  ...
  if(nrow(qual) > numgroup){
    for(k in 1:dividend){
      plot(x=0, y=0,
           ...
           ylim=ifelse(freq, c(0,1), c(lowest.count, highest.count)),
      ...
      legend(x=legend.x, y=legend.y,
             legend=rownames(qual)[((k-1)*numgroup+1):(k*numgroup)],
             cex=0.7, col=1:numgroup, pch=1:numgroup, bty="n")
    }
  }
  plot(x=0, y=0,
       xlim=c(lowest.qual, highest.qual),
  ...
  axis(side=4)
  legend(x=legend.x, y=legend.y,
         legend=rownames(qual)[(dividend*numgroup+1):nrow(qual)],
         cex=0.7, col=1:l, pch=1:l, bty="n")
}
...
##' @param numgroup max number of files plotted on the same graph (default = 25)
...
plot.adp.content <- function(adp.content,
  ...
  if(nrow(adp.content)> numgroup){
    for(k in 1:dividend){
      ...
      axis(side=4)
      legend(x=legend.x, y=legend.y,
             legend=rownames(adp.content)[((k-1)*numgroup+1):(k*numgroup)],
    ...
    axis(side=4)
    legend(x=legend.x, y=legend.y,
           legend=rownames(adp.content)[(dividend*numgroup+1):nrow(adp.content)],
...
plot.seq.length <- function(seq.length,
  ...
  if(nrow(seq.length)> numgroup){
    for(k in 1:dividend){
      ...
      axis(side=4)
      legend(x=legend.x, y=legend.y,
             legend=rownames(seq.length)[((k-1)*numgroup+1):(k*numgroup)],
             col=1:numgroup, pch=1:numgroup, bty="n", cex=0.7)
    }
  }
  plot(x=0, y=0,
  ...
  axis(side=4)
  legend(x=legend.x, y=legend.y,
         legend=rownames(seq.length)[(dividend*numgroup+1):nrow(seq.length)],
         col=1:l, pch=1:l, bty="n", cex=0.7)
}
